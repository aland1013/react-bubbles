...
A token is a string of characters issued by a server to a client, which the client stores in local or session storage. Once the client has a token, it can add Authorization: <token> to request headers to access any resources that the server is protecting by requiring authentication.

- [ ] What steps can you take in your web apps to keep your data secure?

You can create protected routes that only render after authentication by the server, and require authentication headers for any API calls to the server.

- [ ] Describe how web servers work.

Web servers store files for websites and retrieve them and send them to users upon request for a matching URL.
